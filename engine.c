...
/* Write pointer */
static ENGINE_uartWriteHandler transmit_char_via_uart;
...
uint8_t engine_initiate(ENGINE_uartWriteHandler tmp_write_handler, ENGINE_uartReadHandler tmp_read_handler, 
    ...
    //TODO: Check for null in function pointers and take appriopate actions e.g. shut down system
    if(!(tmp_write_handler && tmp_read_handler && tmp_count_unread_chars_handler && tmp_delay_in_ms))
        return 0;
    
    transmit_char_via_uart = tmp_write_handler;
...
void transmit_string_via_uart(const char* str){
    while(*str){
        transmit_char_via_uart(*str++);
    }
}

static void retrieve_uart_unread_chars(char *const buffer)
{
...
void execute_atcmd(const char *atcmd, ...)
{
    ...
    transmit_string_via_uart(atcmd);
    transmit_char_via_uart(13);
    transmit_char_via_uart(10);
...
//TODO: Implement new version of this function that looks for +IPD within whole UART buffer and not just portion with unread characters
